 ...
 */
export function decodeWslListOutput(buffer: Buffer): string {
 ...
 */
export function parseWslList(text: string): WslDistribution[] {
  const distributions: WslDistribution[] = []
  for (const rawLine of text.split(/\r?\n/)) {
    const line = rawLine.replaceAll('\0', '').trimEnd()
    if (line.length === 0 || HEADER.test(line.trim())) continue
    const match = ROW.exec(line)
    const name = match?.[2]
    const stateCell = match?.[3]
    if (match === null || name === undefined || stateCell === undefined) continue
    const versionNumber = Number(match[4])
    const version: 1 | 2 = versionNumber === 1 ? 1 : 2
    const stateRaw = stateCell.toLowerCase()
    const state: WslDistribution['state'] = stateRaw === 'running'
      ? 'running'
      : stateRaw === 'stopped'
        ? 'stopped'
        : 'unknown'
