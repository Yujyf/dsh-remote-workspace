...
const SCHEME_TYPES = new Set<WorkspaceTargetType>([
  'local',
  'wsl',
  'ssh',
  'docker',
...
export function normalizeWindowsPath(path: string): string {
  ...
  if (posixRest === '/') return `${upperDrive}:\\`
 ...
 */
export function windowsPathToUriPath(windowsPath: string): string {
  const canonical = normalizeWindowsPath(windowsPath)
  if (canonical.length === 3) return `/${canonical[0]}:/`
 ...
 * @returns the canonical Windows path.
 */
export function uriPathToWindowsPath(uriPath: string): string {
 ...
 */
export function formatWorkspaceUri(type: WorkspaceTargetType, authority: string, path: string): string {
  const encodedAuthority = encodeURIComponent(authority)
  if (type === 'local') return `local://${encodedAuthority}${windowsPathToUriPath(path)}`
...
export function parseWorkspaceUri(value: string): WorkspaceUri {
  ...
  try {
    ...
    throw new WorkspaceError(
      `workspace URI authority is not valid percent-encoding: '${value}'`,
      'UNSUPPORTED',
      { uri: value },
      { cause: error },
    )
 ...
 */
export function workspaceUriEquals(left: string, right: string): boolean {
 ...
 * @returns the canonical URI record.
 */
export function workspaceUriFromNative(
 ...
 */
export function workspaceUriFromWslUnc(unc: string): WorkspaceUri | undefined {
 ...
 */
export function defaultRemoteWorkspaceTitle(uri: WorkspaceUri): string {
  if (uri.type === 'local') {
