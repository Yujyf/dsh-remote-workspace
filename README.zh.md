...
- **不绑定** —— 保持 DSH 自带的沙箱本地后端不变；会话在绑定工作区之前都走这里。
- **WSL 目标** —— 通过 `wsl.exe` 进入某个发行版；文件与进程操作都在该发行版内执行，使用它自己的 POSIX 路径、HOME 和工具链。

侧边栏新增一个入口用于选择：挑目标、浏览文件夹、登记为工作区，并绑定到当前会话。绑定之后，该会话的 shell 命令、文件读写与文件搜索都在那里执行，**工作目录就是绑定的目录**，并且会通过一条运行时上下文告诉模型它身处哪个世界、应当使用 `bash`。

选择器只列出**远程**执行世界。宿主自身所在的世界不会出现——没有绑定工作区的会话本来就在那里执行；在 Windows 宿主上，列表就是每个 WSL 发行版一行。DSH 自己的工作区以 **本地** 分组并排显示，顶部的执行世界筛选可以把列表收窄到某一个世界。

每个远程工作区同时会登记为一个 **DSH 工作区**：路径用发行版的 UNC 视图（`\\wsl.localhost\Ubuntu-26.04\home\me\project`），标题为 `<名称> · WSL <发行版>`。这条登记才是让它出现在 DSH 自己的工作区列表、并被其它读 `ctx.workspaceRegistry` 的插件看到的东西，而不是只存在于本包自己的注册表；删除本包的登记会一并删掉它。在 DSH 自己的界面里于该工作区下新建的会话，不需要任何绑定就直接在该发行版中执行——会话目录本身已经指明了世界。

**回到本地执行** 解除绑定：会话回到宿主上运行，不删除任何东西。
...
| Node | `^22.19 \|\| >=24` |
| 操作系统 | WSL 目标需要 Windows 10/11；在 macOS 与 Linux 上选择器没有可用的远程世界 |
| WSL | `wsl.exe` 在 `PATH` 中，且至少注册了一个发行版 |

WSL 目标只在 Windows 上枚举（`process.platform === 'win32'`）。其他平台的远程目标列表为空，会话继续在宿主世界里执行。
...
1. 在 Web GUI 中打开一个会话。
2. 点击侧边栏底部的 **远程工作区**。列表里是 **本地**（DSH 自己的工作区）加上每个 WSL 发行版；上方的筛选行可以把列表收窄到某一个世界。
3. 选择一个 WSL 发行版，状态点会显示 `可用`、`启动中`、`已停止` 或 `不可用`；连接一个 `已停止` 的发行版会把它启动起来。
4. 点击 **浏览文件夹…**，进入项目目录，选择 **使用此文件夹**。该文件夹即被登记为工作区、连接，并以 `<名称> · WSL <发行版>` 加进 DSH 自己的工作区列表。
5. 在工作区行上点击 **在此会话中使用**。面板标题随后显示 *当前会话在此执行 · Ubuntu-26.04 · /home/me/project*，该会话的 `bash`、读文件、改文件就在该目录下执行；相对路径以它为基准，并且下一次请求会告诉模型所处世界、工作目录以及应当使用 `bash` 而不是 `pwsh`。
6. **回到本地执行** 解除绑定；**删除** 只删除登记记录，不会删除文件夹或其中的文件。

未绑定任何工作区的会话继续在宿主上执行。绑定是按会话生效的，所以同时打开的两个会话可以分别运行在两个不同的世界里。
...
DSH 在创建会话时就固定了它的工作目录，而且该 header 不可变。所以绑定不改写会话，而是由路由器逐次翻译：

- 会话自己的宿主目录（`E:\work\project`）变成绑定的工作区目录（`/home/me/project`），默认或相对路径的工具调用就落在这里；
- 其他宿主路径保留盘符映射（`C:\Users\me` 即 `/mnt/c/Users/me`），显式传入的 `workdir` 依然能到达它指名的宿主文件；
- 仅在会话已绑定时注入一条运行时上下文，说明发行版、工作目录，并说明应当使用 `bash`——WSL 里没有 PowerShell。

会话记录的宿主目录取自当前活动会话；绑定本身是持久的，重启后仍然生效。
...
- **那条 DSH 工作区登记携带的是 UNC 路径。** DSH 的工作区注册表只接受宿主进程能解析的目录，所以登记进去的是 `\\wsl.localhost\<发行版>\…`；在那里新建的会话记录的目录也是它。因此登记时发行版必须处于运行状态（流程会先连接），而登记之后把发行版停掉，DSH 会认为该工作区的目录无法验证。
- **每个会话只有一个绑定。** 会话中途可以改绑；旧世界里的进程不会被迁移。
- **会话 header 里的目录仍是宿主路径。** DSH 没有提供改写它的接口，因此系统提示里仍会出现宿主路径，而工具实际解析到绑定的世界；靠那条运行时上下文保持两者一致。
- **绑定后 `pwsh` 无法运行。** WSL 内不存在 PowerShell，上下文会引导模型改用 `bash`；工具本身留在 schema 里是因为 DSH 按宿主平台选择工具。
